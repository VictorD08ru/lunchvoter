...
import tk.djandjiev.lunchvoter.backend.util.exception.ErrorInfo;
...
import static tk.djandjiev.lunchvoter.backend.util.exception.ErrorType.*;
...
public class ExceptionInfoHandler {
    ...
    public static final String EXCEPTION_DUPLICATE_MENU_ITEM = "Same dish for this restaurant already exists";
    public static final String EXCEPTION_RESTAURANT_UNIQUE_NAME = "Restaurant name must be unique";

    private static final Map<String, String> constraintsMap = Collections.unmodifiableMap(
            new HashMap<String, String>() {
                {
                    put("users_unique_email_idx", EXCEPTION_DUPLICATE_EMAIL);
                    put("menu_unique_dish_per_restrnt_idx", EXCEPTION_DUPLICATE_MENU_ITEM);
                    put("restaurants_unique_name_idx", EXCEPTION_RESTAURANT_UNIQUE_NAME);
                    put("votes_unique_user_date_idx", "Error of updating user\'s vote");
                }
            });

    @ExceptionHandler(ApplicationException.class)
    public ResponseEntity<ErrorInfo> applicationError(HttpServletRequest req, ApplicationException appEx) {
        ErrorInfo errorInfo = logAndGetErrorInfo(req, appEx, false, appEx.getType(), appEx.getArgs());
